...
function AppShell() {
  const { currentUser, authChecked, loading, logout } = useData();
  ...
  // Waiting to find out if a stored token is still valid.
  if (!authChecked) {
    return (
  ...
  // Logged in but CRM data hasn't finished loading yet.
  if (loading) {
    return (
      <div className="app-loading">
        <div className="app-loading-mark">PIT</div>
